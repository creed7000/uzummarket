...
import './components/MobileBottomNav/MobileBottomNav.css';
...
export default function App() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (view === 'category') {
      ...
      // Narx filtri
      result = result.filter(p => p.price >= filters.minPrice && (filters.maxPrice > 0 ? p.price <= filters.maxPrice : true));
      // Dinamik filtrlar
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-[#1f2026] font-sans selection:bg-purple-100 flex flex-col">
      ...
      <header className="bg-white sticky top-0 z-50 shadow-sm">
        <div className="bg-[#f2f4f7] py-1.5 hidden lg:block border-b border-gray-200">
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center gap-4 md:gap-6">
          <div className="flex items-center gap-2 cursor-pointer group" onClick={() => setView('home')}>
            <div className="w-8 h-8 md:w-10 md:h-10 bg-[#7000ff] rounded-full flex items-center justify-center text-white text-lg md:xl font-bold group-hover:rotate-12 transition-transform">U</div>
            <span className="text-xl md:text-2xl font-bold text-[#7000ff] hidden sm:block">uzum market</span>
          </div>

          <button onClick={() => showToast("Katalog menyusi yangilanmoqda...")} className="bg-[#f0edff] text-[#7000ff] px-3 py-2 md:px-4 md:py-2.5 rounded-lg flex items-center gap-2 font-semibold hover:bg-[#e5e0ff] transition-colors hidden md:flex">
            <Menu size={20} /> Katalog
          </button>

          <div className="flex-grow relative order-3 md:order-none w-full md:w-auto">
            <input 
              type="text" 
              placeholder="Mahsulotlar qidirish..." 
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full border border-gray-200 rounded-lg py-2 md:py-2.5 pl-4 pr-12 outline-none focus:border-[#7000ff] bg-gray-50 transition-all text-sm md:text-base"
            />
            <button className="absolute right-0 top-0 h-full px-4 md:px-5 text-gray-400 hover:text-[#7000ff]"><Search size={20}/></button>
          </div>

          <div className="flex items-center gap-4 md:gap-6 order-2 md:order-none">
            <div className="flex flex-col items-center cursor-pointer text-gray-700 hover:text-[#7000ff] hidden md:flex" onClick={() => setModals({...modals, login: true})}>
              <User size={22} />
              <span className="text-[11px] mt-1 font-medium">Kirish</span>
            </div>
            <div className="flex flex-col items-center cursor-pointer text-gray-700 hover:text-[#7000ff] relative hidden md:flex" onClick={() => setModals({...modals, fav: true})}>
            ...
            <div className="flex flex-col items-center cursor-pointer text-gray-700 hover:text-[#7000ff] relative" onClick={() => setModals({...modals, cart: true})}>
              <ShoppingBag size={22} />
              <span className="text-[11px] mt-1 font-medium hidden md:block">Savat</span>
              {cartCount > 0 && (
            ...
            {/* Mobile Menu Icon */}
            <div className="md:hidden flex flex-col items-center cursor-pointer text-gray-700 active:text-[#7000ff]" onClick={() => showToast("Menyu ochilmoqda...")}>
              <Menu size={22} />
            </div>
          </div>
        </div>
            ...
            {CATEGORIES.map(cat => (
              <div 
                ...
                onClick={() => { 
                  ...
                  setView('category'); 
                  setFilters({ minPrice: 0, maxPrice: 100000000, ram: [], size: [] });
                  window.scrollTo(0, 0); 
                }} 
              ...
              >
                {cat.icon && <CategoryIcon name={cat.icon} size={20} />} {cat.name}
              </div>
            ))}
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 flex-grow">
        {view === 'home' ? (
          <section className="animate-in fade-in duration-500">
            {/* HERO BANNER */}
            <div className="rounded-2xl md:rounded-3xl overflow-hidden mb-6 md:mb-12 h-[200px] md:h-[380px] relative shadow-lg md:shadow-2xl group cursor-pointer">
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/40 to-transparent flex flex-col justify-center px-6 md:px-16 text-white">
                <span className="bg-yellow-400 text-black font-bold px-3 py-1 md:px-4 md:py-1.5 rounded-full text-[10px] md:text-xs w-fit mb-3 md:mb-6 tracking-wider">SUPER CHEGIRMA</span>
                <h2 className="text-2xl md:text-6xl font-black mb-2 md:mb-6 leading-tight">Yozgi Katta <br /> Sotuvlar!</h2>
                <p className="text-sm md:text-xl opacity-90 mb-4 md:mb-10 max-w-xs md:max-w-lg font-medium">Sevimli mahsulotlaringizni 50% gacha foyda bilan xarid qiling.</p>
                <button className="bg-white text-[#7000ff] px-6 py-2 md:px-10 md:py-4 rounded-xl md:rounded-2xl font-black text-xs md:text-base w-fit hover:scale-105 active:scale-95 transition-all shadow-xl">HOZIR XARID QILISH</button>
              </div>
            </div>
          ...
          <section className="animate-in fade-in duration-500">
            ...
            <h1 className="text-3xl font-bold mb-8 text-[#1f2026] flex items-center gap-3">
              {activeCategoryData?.icon && <CategoryIcon name={activeCategoryData.icon} size={32} className="text-[#1f2026]" />} {activeCategoryData?.name}
            </h1>
          ...
          <section className="animate-in slide-in-from-bottom-10 duration-500">
            ...
            {selectedProduct && (
              <div className="bg-white rounded-2xl md:rounded-[40px] p-4 md:p-10 shadow-sm flex flex-col lg:flex-row gap-8 md:gap-16 min-h-[500px] md:min-h-[600px] border border-gray-100">
                <div className="flex-1 overflow-hidden rounded-2xl md:rounded-[32px] bg-gray-50 group">
                  <img src={selectedProduct.img} alt={selectedProduct.name} className="w-full h-full max-h-[400px] md:max-h-[600px] object-cover hover:scale-105 transition-transform duration-700" />
                </div>
                <div className="flex-1 space-y-6 md:space-y-10 py-2 md:py-4">
                  <div>
                    ...
                    <h1 className="text-2xl md:text-5xl font-black leading-tight mb-4 md:mb-6 tracking-tight">{selectedProduct.name}</h1>
                  </div>
                  
                  <div className="space-y-2">
                    <span className="text-gray-400 line-through text-lg md:text-2xl font-medium">{selectedProduct.oldPrice.toLocaleString()} so'm</span>
                    <div className="flex items-center gap-4 md:gap-6">
                      <span className="text-3xl md:text-5xl font-black text-[#7000ff]">{selectedProduct.price.toLocaleString()} so'm</span>
                      <span className="bg-red-500 text-white px-3 py-1 md:px-4 md:py-1.5 rounded-lg md:rounded-xl text-xs md:text-sm font-black animate-bounce whitespace-nowrap">Foyda</span>
                    </div>
                  </div>
      ...
      {/* FOOTER SECTION */}
      <footer className="bg-white border-t border-gray-100 pt-8 md:pt-12 pb-24 md:pb-8 mt-8 md:mt-16">
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12 mb-12">
            <div>
              <h4 className="font-bold text-sm md:text-lg mb-4 md:mb-6">Biz haqimizda</h4>
              <ul className="space-y-2 md:space-y-3 text-gray-400 md:text-gray-500 font-medium text-xs md:text-base">
            ...
            <div>
              <h4 className="font-bold text-sm md:text-lg mb-4 md:mb-6">Foydalanuvchilarga</h4>
              <ul className="space-y-2 md:space-y-3 text-gray-400 md:text-gray-500 font-medium text-xs md:text-base">
            ...
            <div className="hidden md:block">
              <h4 className="font-bold text-sm md:text-lg mb-4 md:mb-6">Tadbirkorlarga</h4>
              <ul className="space-y-2 md:space-y-3 text-gray-400 md:text-gray-500 font-medium text-xs md:text-base">
            ...
            <div className="col-span-2 md:col-span-1">
              <h4 className="font-bold text-sm md:text-lg mb-4 md:mb-6">Ilovani yuklab olish</h4>
              <div className="flex gap-4 mb-8">
                <a href="https://apps.apple.com/uz/app/uzum-market-internet-magazin/id1640997456" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
                   <img src="https://upload.wikimedia.org/wikipedia/commons/3/3c/Download_on_the_App_Store_Badge.svg" className="h-8 md:h-10" alt="AppStore" />
                </a>
                <a href="https://play.google.com/store/apps/details?id=uz.uzum.app" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
                   <img src="https://upload.wikimedia.org/wikipedia/commons/7/78/Google_Play_Store_badge_EN.svg" className="h-8 md:h-10" alt="GooglePlay" />
                </a>
              </div>
              <h4 className="font-bold text-sm md:text-lg mb-4">Uzum ijtimoiy tarmoqlarda</h4>
              <div className="flex gap-3 md:gap-4">
                <a href="https://instagram.com/k.habibullayvv" target="_blank" rel="noopener noreferrer" className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center rounded-lg bg-gray-50 hover:bg-gray-100 cursor-pointer text-pink-600 transition-colors">
                  <Instagram size={20} />
                </a>
                <a href="https://t.me/hbbllyvv7" target="_blank" rel="noopener noreferrer" className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center rounded-lg bg-gray-50 hover:bg-gray-100 cursor-pointer text-blue-500 transition-colors">
                  <Send size={20} />
                </a>
                <a href="https://facebook.com" target="_blank" rel="noopener noreferrer" className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center rounded-lg bg-gray-50 hover:bg-gray-100 cursor-pointer text-blue-700 transition-colors">
                  <Facebook size={20} />
                </a>
                <a href="https://youtube.com" target="_blank" rel="noopener noreferrer" className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center rounded-lg bg-gray-50 hover:bg-gray-100 cursor-pointer text-red-600 transition-colors">
                  <Youtube size={20} />
                </a>
              </div>
            </div>
      ...
      <MobileBottomNav 
        activeTab={modals.cart ? 'cart' : modals.fav ? 'fav' : modals.login ? 'login' : view}
        onViewChange={(newView) => { 
          setView(newView); 
          setModals({ cart: false, fav: false, login: false });
          window.scrollTo(0, 0); 
        }}
        onModalOpen={(name) => {
          setModals({ cart: false, fav: false, login: false, [name]: true });
        }}
        cartCount={cartCount}
...
function ProductCard({ product, isFavorite, onDetail, onToggleFav, onAddToCart }) {
  return (
    <div className="bg-white rounded-2xl md:rounded-3xl overflow-hidden flex flex-col cursor-pointer transition-all duration-300 relative border border-gray-100 group hover:md:-translate-y-2 hover:md:shadow-[0_20px_40px_rgba(0,0,0,0.08)]" onClick={onDetail}>
      <div className="relative h-48 md:h-72 bg-gray-50 overflow-hidden">
        {product.badge && (
          <span className="absolute top-2 left-2 md:top-4 md:left-4 bg-[#7000ff] text-white text-[8px] md:text-[10px] px-2 py-1 md:px-3 md:py-1.5 rounded-full font-black z-10 shadow-lg tracking-widest">{product.badge}</span>
        )}
        <button 
          onClick={(e) => { e.stopPropagation(); onToggleFav(); }} 
          className="absolute top-2 right-2 md:top-4 md:right-4 w-8 h-8 md:w-11 md:h-11 rounded-full bg-white/90 backdrop-blur-sm flex items-center justify-center hover:scale-110 active:scale-90 z-10 transition-all shadow-md group/btn"
        >
          <Heart className={`transition-all duration-300 ${isFavorite ? 'fill-red-500 text-red-500 scale-110' : 'text-gray-300 group-hover/btn:text-red-300'}`} size={18} />
        </button>
        <img src={product.img} className="w-full h-full object-cover group-hover:md:scale-110 transition-transform duration-1000" alt={product.name} />
      </div>
      
      <div className="p-3 md:p-5 flex flex-col flex-grow">
        <h3 className="text-xs md:text-[15px] font-bold text-gray-800 line-clamp-2 h-8 md:h-11 mb-2 md:mb-3 leading-tight group-hover:text-[#7000ff] transition-colors">{product.name}</h3>
        
        <div className="text-[10px] md:text-[12px] mb-3 md:mb-5 flex items-center gap-1 md:gap-1.5">
          <Star size={12} className="fill-yellow-400 text-yellow-400" />
          <span className="text-gray-800 font-bold">{product.rating}</span> 
          <span className="text-gray-400 font-medium hidden sm:inline">({product.reviews} ta sharh)</span>
        </div>
        
        <div className="bg-yellow-100/60 text-[#1f2026] text-[9px] md:text-[11px] font-bold px-2 py-1 md:px-3 md:py-1.5 rounded-lg md:rounded-xl w-fit mb-4 md:mb-6">
          {Math.round(product.price / 12).toLocaleString()} so'm/oy
        </div>
        
        <div className="mt-auto pt-2 md:pt-4 border-t border-gray-50">
          <div className="text-[10px] md:text-[13px] text-gray-300 line-through mb-0.5 md:mb-1 font-bold">{product.oldPrice.toLocaleString()} so'm</div>
          <div className="flex justify-between items-center">
            <span className="font-black text-sm md:text-[19px] tracking-tight">{product.price.toLocaleString()} so'm</span>
            <button 
              onClick={(e) => { e.stopPropagation(); onAddToCart(); }} 
              className="w-8 h-8 md:w-12 md:h-12 rounded-lg md:rounded-2xl bg-gray-50 text-gray-400 flex items-center justify-center hover:bg-[#7000ff] hover:text-white transition-all md:hover:shadow-lg md:hover:shadow-purple-200 active:scale-90"
            >
              <ShoppingBag size={16} />
            </button>
          </div>
